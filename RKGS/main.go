...
func job_splitter(thread_splitter_buffer chan Job){
    for{
        ...
        if v.Work == "generate"{
            ...
            go check_if_url_exists_db(v,job)
        }
    }   
}
...
func check_url_is_unique_db(salted_url string) (bool,error){
    ...
    if err==nil{
        ...
        Check_error(err)
        return true,nil //unsafe, even if there is error or the reponse is empty we are sending back nil
    }
}
...
func check_if_url_exists_db(job_obj Job_split, job_raw Job){
    salted_url := job_obj.Url+"_"+job_obj.Pool;
